...
{
    /// <summary>
    /// Контролирует игрока через клавиши WASD/стрелки:
    /// </summary>
    ...
    public class PlayerKeyboardController : MonoBehaviour
    {
        ...
        private static readonly string[] IdleAnimations = { "Idle_02_003" };
        ...
        [SerializeField, Tooltip("Дальность кувырка")] 
        private float rollDistance = 6f;
        [SerializeField, Tooltip("Кулдаун между кувырками")] 
        private float rollCooldown = 2f;
        [SerializeField, Tooltip("Множитель скорости при кувырке"), Range(0.1f, 3f)]
        ...
        private void Awake()
        {
            ...
            if (skeletonAnimation == null)
            {
                Debug.LogError("PlayerKeyboardController: SkeletonAnimation не назначен", this);
                return;
            }
        ...
        private void UpdateMovementAnimation(Vector2 dir)
        {
            ...
            {
                if (!idleCycling) PlayIdleSequence();
                return;
            }
            idleCycling = false;
        ...
        private void UpdateTrailEffects(Vector2 dir)
        {
            ...
            for (int i = 0; i < trailEmissions.Length; i++)
            {
                ...
                if (running)
                {
                    var baseVel = -new Vector3(dir.x, dir.y, 0f) * currentSpeed * trailVelocityMultiplier;
        ...
        public void ApplySlow(float factor, float duration)    => RestartCoroutine(ref slowCoroutine, SlowCoroutine(factor, duration));
        ...
        public void IncreaseSpeed(float mult)                 => StartCoroutine(SpeedBoostCoroutine(mult, 5f));
        public void ReceiveDarkSkullHit()                     { if (++darkSkullHitCount >= maxDarkSkullHits) Die(); }
        ...
        private IEnumerator StunCoroutine(float duration)    => BindingCoroutine(duration);
        ...
        private void RestartCoroutine(ref Coroutine coro, IEnumerator routine)
        {
            if (coro != null) StopCoroutine(coro);
            coro = StartCoroutine(routine);
        }
        ...
        {
            if (Array.IndexOf(IdleAnimations, animName) < 0) idleCycling = false;
